...
class ImageService {
  ...
  public async createImages({ userId, files }: CreateImagesDTO): Promise<Images> {
    try {
      ...
      const createdImage: Images = await this._createImageInfo({ userId, keys, urls });
  ...
  public async updateImages({ userId, files }: UpdateImagesDTO): Promise<Images> {
    try {
      ...
      //메타데이터 업데이트
      const updatedImages: Images = await this._updateImageInfo({ userId, keys: newKeys, urls: newUrls });
   ...
   */
  private async _uploadImageToStorage(buffer: Buffer, key: string): Promise<{
    url: string;
    key: string;
  }> {
    const params = {
    ...
    await Promise.all(
      keys.map(key => {
        ...
        try {
          ...
          throw { message: 'storage에 있는 이미지 삭제 실패했습니다.', error: error.toString() }
        }
      })
    )
  }
