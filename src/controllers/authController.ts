...
class AuthController {
  ...
  public async onBoard({ body }: Request, res: Response) {
    ...
    try {
      ...
      if (
        ...
        error instanceof InvalidSocketIdException ||
  ...
  //  * /api/auth/signin:
  //  *   post:
  //  *     summary: 접속 시 토큰 존재여부를 확인 후, 유효한 accessToken일 경우 accessToken를 갱신
  ...
  //  *        `refreshToken expiration -> 30년`<br><br><br>
  //  *
  //  *        `[Header]`<br>
  ...
  //   try {
  //     const { accessToken } = await AuthService.signIn(fetchOrGenerateToken);
   ...
   *        만약 헤더에 BearerToken이 없는 상태로 호출하게 되면 <br>
   *        401과 함께 MissingTokenException을 response받습니다. <br>
   *        이 경우에는 refreshToken을 넣고 재요청을 하거나<br>
   *        onboard를 통해 onBoarding을 시도해야합니다. <br><br>
   *
   *        `case 2)` <br>
   *        만약 헤더에 유효한 refreshToken을 넣고 호출하게 되면 <br>
   *        새로 갱신받은 accessToken을 response받습니다.<br><br>
   *
   *        `case 3)` <br>
   *        만약 헤더에 유효하지 않은 refreshToken을 넣고 호출하게 되면 <br>
   *        401과 함께 InvalidTokenException을 response받습니다.<br>
   *        이 경우에는 유효한 accessToken을 통해 재요청을 하거나 <br>
   *        onboard를 통해 onBoarding을 시도해야합니다.<br><br>
   *
  ...
  public async renew({ headers, userId }: Request, res: Response) {
    ...
    try {
      const { accessToken } = await AuthService.renew(renewTokenDTO);
